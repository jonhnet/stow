...
type Preview = { itemId: string; memberIds: string[]; text: string; x: number; y: number; width: number; target: DropTarget | null };
...
function childDepth(drag: Drag) {
  return drag.parentId ? drag.x - drag.startX > -INDENT : drag.x - drag.startX >= INDENT;
}

/** Resolve a visible insertion slot; roots always move past whole parent groups. */
function dropTarget(drag: Drag): DropTarget | null {
  ...
  const rows = [...drag.group.querySelectorAll<HTMLElement>('[data-check-row]')].filter(row => !drag.memberIds.includes(row.dataset.checkRow!));
  ...
  if (childDepth(drag)) {
    if (drag.memberIds.length > 1 || slot === 0) return null;
    const previous = rows[slot - 1];
    return { id: previous.dataset.checkRow!, placement: 'after', parentId: previous.dataset.rootId!, markerId: previous.dataset.checkRow!, markerPlacement: 'after' };
  }
  const next = rows[slot];
  const target = next ?? rows[rows.length - 1];
  const after = !next || !!next.dataset.parentId;
  const rootId = target.dataset.rootId!;
  const marker = after ? rows.filter(row => row.dataset.rootId === rootId).at(-1)! : target;
  return { id: rootId, placement: after ? 'after' : 'before', parentId: null, markerId: marker.dataset.checkRow!, markerPlacement: after ? 'after' : 'before' };
}
...
const ChecklistRow = memo(function ChecklistRow({ item, root, disabled, preview, actions }: {
  item: Item; root: Item; disabled: boolean; preview: Preview | null; actions: RefObject<RowActions>;
    ...
    return <div key={item.id} role="listitem" data-check-row={item.id} data-root-id={root.id} data-parent-id={parentId} style={style} className={`editor-check-row ${parentId ? 'is-child' : ''} ${item.checked ? 'is-checked' : ''} ${preview?.memberIds.includes(item.id) ? 'drag-source' : ''} ${insertion}`}>
...
function EditorChecklist({ note, disabled, onAddItem }: { note: Note; disabled: boolean; onAddItem: (text: string) => string }) {
  ...
  const updatePreview = (drag: Drag) => {
    ...
    setPreview({
      itemId: drag.itemId, memberIds: drag.memberIds, text: drag.text,
      x: drag.x, y: drag.y,
  ...
  const finishDrag = (commit: boolean) => {
    ...
    if (target) {
      ...
      setAnnouncement(parent ? `Moved ${drag.text} under ${parent.text || 'an empty item'}.` : `Moved ${drag.text}${drag.memberIds.length > 1 ? ` with its ${drag.memberIds.length - 1} child items` : ''} to the top level.`);
    } else if (drag.moved || !commit) setAnnouncement(commit && childDepth(drag) && drag.memberIds.length > 1 ? 'A group with child items cannot be nested.' : `Reordering ${drag.text} canceled.`);
  ...
  useEffect(() => {
    ...
    const members = current ? parentId ? [drag.itemId] : [current.root.id, ...current.children.map(child => child.id)] : [];
    const membershipChanged = members.length !== drag.memberIds.length || members.some(id => !drag.memberIds.includes(id));
    if (disabled || !current || membershipChanged || isChecklistGroupChecked(current) !== drag.checked || parentId !== drag.parentId || (drag.checked && !showCompleted)) finishDrag(false);
  ...
  const startDrag = (event: ReactPointerEvent<HTMLButtonElement>, item: Item) => {
    ...
    dragRef.current = {
      ...
      memberIds: parentId ? [item.id] : [item.id, ...family.children.map(child => child.id)],
      pointerId: event.pointerId, handle, group, scroller: scrollContainer(group),
      startX: event.clientX, startY: event.clientY,
      x: event.clientX, y: event.clientY, moved: false,
    };
    setAnnouncement(`Reordering ${item.text || 'list item'}${!parentId && family.children.length ? ` with ${family.children.length} child items` : ''}. Drag left or right to change nesting. Press Escape to cancel.`);
    frameRef.current = requestAnimationFrame(autoScroll);
  };
  ...
  const indent = (item: Item, outdent: boolean) => {
    ...
    } else if (!outdent && groups.some(group => group.root.id === item.id && group.children.length)) setAnnouncement('A group with child items cannot be nested.');
    return changed;
  };
  ...
  return <div ref={ref} className={`editor-checklist ${preview ? 'is-reordering' : ''}`}>
