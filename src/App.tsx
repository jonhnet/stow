...
export default function App() {
  ...
  const { notes, labels, status, ready, access, accessMessage, authMode, user, canReload, error, canUndo, canRedo, pending, localPending, images } = useStow();
  ...
  if (access !== 'ready') {
    ...
    if (access === 'opening' && !accessMessage) return <div className="loading-state" role="status" aria-label="Loading"><LoaderCircle size={28} className="spinning" aria-hidden="true" />{error && <p role="alert">{error}</p>}</div>;
  ...
  const syncLabel = error || (localPending > 0 ? 'Saving on this device…' : status === 'online' ? pending ? 'Syncing…' : imageStatus || 'Connected' : status === 'connecting' ? 'Connecting…' : status === 'locked' ? 'Locked' : status === 'error' ? 'Could not sync' : 'Offline — notes stored on this device');
  return <div className={`app ${sidebar ? 'sidebar-open' : 'sidebar-closed'}`}>
    ...
    {toast && <div className="toast" role="status" aria-atomic="true"><span>{toast.message}</span><IconButton label="Dismiss notification" onClick={() => setToast(null)}><X size={18} /></IconButton></div>}
  </div>;
}
