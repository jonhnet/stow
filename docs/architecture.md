...
Current data, session Undo, and saved versions have separate lifetimes. Every text input changes `Y.Text`, enters local IndexedDB, and synchronizes immediately. A local `Y.UndoManager` retains fine steps: 500 ms idle, two seconds continuous input, cursor movement, target changes, or insertion/deletion direction changes split typing. Paste, cut, and IME composition remain whole operations. Remote changes and timestamp bookkeeping do not enter the local Undo stack.
...
IndexedDB schema 1 has `updates`, `pendingEdits`, and `maintenance` stores. A dedicated worker appends and compacts current updates atomically with pending timestamps. `maintenance` records the validated update-log tail, avoiding repeated deletion compaction on unchanged reloads. The latter contains only source IDs and pending modification timestamps, atomically saved with current updates. Account-scoped writer Web Locks prevent recovery of another active tab's timestamps. Recovery publishes those timestamps without reconstructing, replaying, or uploading historical note snapshots. Permanent deletion prunes obsolete recovery entries. Failed writes remain visible and retryable. Older historical cache schemas are rejected explicitly; there is no automatic migration.
