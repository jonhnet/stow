...
test('overview downloads thumbnails; originals open on demand and report offline availability', async ({ browser }) => {
  ...
  try {
    ...
    await card(remote).getByRole('button', { name: 'Open original: drawing-1.png', exact: true }).click();
    ...
    await expect(card(remote).getByRole('img', { name: 'drawing-1.png', exact: true })).toBeVisible();
    await card(remote).getByRole('button', { name: 'Open original: drawing-1.png', exact: true }).click();
    await expect(remote.getByRole('dialog').getByRole('img', { name: 'drawing-1.png', exact: true })).toBeVisible();
    await remote.getByRole('button', { name: 'Close image' }).click();
    await card(remote).getByRole('button', { name: 'Open original: drawing-2.png', exact: true }).click();
    await expect(remote.getByRole('dialog')).toContainText('Connect to download it.');
    await expect(remote.getByRole('dialog').locator('img')).toHaveCount(0);
